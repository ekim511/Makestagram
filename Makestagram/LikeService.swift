...
struct LikeService {
    ...
    static func create(for post: Post, success: @escaping (Bool) -> Void) {
        ...
        // 2 We create a reference to the current user's UID
    ...
    //make sure that the post has a key, build a relative path to the location of where we store the current user's like data for a specific post, if a like were to exist, use a special query that checks whether a value exists at the location that we're reading from. If there is, we know that the current user has liked the post. Otherwise, we know that the user hasn't.
